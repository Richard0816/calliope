...
The popout also overlays the **circular-shift null floor** — the p95/p99 of candidate-peak prominences when each cell's onset train is independently time-shifted so any cross-cell coincidence is pure chance (`utils.circular_shift_null_prominences`, the same engine as `scripts/null_prominence_audit.py`). A real population event must clear that floor, so it's a principled lower bound for `min_prominence`. It's computed off-thread (the popout opens instantly, the lines fill in a few seconds later) and shown as a **reference only — never auto-applied**: an audit across recordings found the null is a near-constant noise floor (~0.0028), while the user's hand-tuned threshold consistently sits above it (well above on event-rich recordings), so the human stays in the loop. (Council decision, 2026-05-29.)
