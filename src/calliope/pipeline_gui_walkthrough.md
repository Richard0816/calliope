...
When reopening a run, the reader tabs (lowpass, clustering) and the `null_prominence_audit.py` script resolve the cell-filter keep mask by **anchoring to the on-disk column count of the filtered memmap** (`utils.resolve_filtered_mask`), not the live `predicted_cell_mask`/`iscell`. This is what lets a run reopen cleanly even if curation drifted the cell mask after the memmaps were written: a live-mask `N_kept` would request a wrong-shaped mapping and Windows would raise `[WinError 8] Not enough memory resources` (lowpass) or the clustering spatial map would fail its `len(stat) != N` guard with **"Spatial unavailable (stat / dF/F mismatch)"**. If the mask drifted past every saved candidate, the loader now raises a clear "re-run Tab 3/Tab 4" error instead of a cryptic WinError.
...
The per-row **Edit parameters** button opens a unified Advanced dialog with every PARAM_SPEC entry from Tabs 1 + 3 + 4 + 5 plus the clustering / xcorr knobs (74 in total), grouped in pipeline order: `1. Preprocess - …` → `2. Detection - …` → `3. Low-pass - …` → `4. Events - …` → `5. Clustering` → `6. Cross-correlation` → `7. Pipeline-wide`. The top-level **Apply settings to selected** button opens the same dialog and writes its output into the params of the **checked** rows only — the same selection the **Merge selected** button reads. A **Select all** button next to it ticks (or unticks) every row's checkbox in one click, so applying settings to the whole queue is just Select all → Apply settings to selected. Per-row **Edit parameters** dialogs are only needed when one recording wants different settings.
...
**Genuinely-noise recordings.** A recording that is just noise can finish detection with zero ROIs surviving the cell filter — `detection_run.compute_filtered_dff` then writes no `r0p7_filtered_dff.memmap.float32` and the lowpass stage has nothing to read. Both runners detect this right after detection (GUI: `_detection_has_no_rois`; headless: `batch_pipeline._no_rois_survive`), skip `lowpass`…`spatial_propagation`, and record the recording with status **`no_rois`** (downstream stages `skipped`). The detection folder + figures that were produced are still finalized to the output dir. Without this guard the GUI batch would stall indefinitely at the lowpass stage on the first noise recording.

**Recordings with no events.** The same stall class hits cross-correlation: when event detection finds zero population events, the xcorr tab's per-event run (`_on_run_per_event`) bails early without publishing `set_xcorr_ready`, so the GUI batch's two-step crosscorrelation counter would hang forever. `_stage_crosscorrelation` now checks `xc._event_windows` after the full-recording xcorr completes and, when empty, advances straight to stage-done — the full xcorr (which needs no events) still runs and its figures are kept. The headless `crosscorrelation.run_crosscorrelation` already guarded this (`if event_windows is not None and len(event_windows) > 0`), so only the GUI driver needed the fix.

**Single-ROI recordings (clustering).** Clustering needs ≥2 ROIs — hierarchical linkage is undefined for one trace (`pdist` on a single column raises *"the number of observations cannot be determined on an empty distance matrix"*). A 1-ROI recording clears detection/lowpass/events but would crash there. After event detection the driver checks `_kept_roi_count(plane0)` and, when `<2`, skips clustering + cross-correlation (marked `skipped`) and jumps straight to spatial propagation, so the row still finishes. `core.clustering._ward_linkage` and the GUI `ward_linkage` also raise a clear `ValueError`, and the headless runner applies the same skip.

**The systemic safety-net (any worker crash fails-and-continues).** Underlying all three cases above: the GUI batch driver triggers each stage's tab worker, then waits on a one-shot AppState "ready" signal. If a worker thread raises *before* publishing that signal — for *any* reason (low data, GPU OOM, a bad TIFF) — there is nothing to wait on and the queue would hang. Each analysis tab's `_on_error` now calls `gui_common.report_stage_error(self.state, msg)`, which publishes the failure on `AppState`'s stage-error channel (and suppresses the tab's blocking modal while `state.batch_active`). The driver subscribes to that channel and calls `_fail_stage` the moment a worker errors, sharing the one-shot guard with the ready path. So the proactive skips above keep the common low-data cases *clean*, while the safety-net guarantees the queue can never silently stall on an unexpected worker failure.

**RAM out-of-memory auto-retry.** Building on that safety-net: when `_fail_stage` sees a memory-allocation error (`_is_oom_error` — `MemoryError` / numpy *"Unable to allocate"* / CUDA-CuPy *"out of memory"*), it doesn't just fail the row — it **re-runs the whole recording from preprocess with progressively lower resource settings**, up to 2 attempts. The conservative profile (`_apply_resource_profile`, applied each stage in `_begin_stage` based on `row._oom_attempt`) turns the suite2p `batch_size` down via the adaptive sizer's `conservative_scale` (×0.5 then ×0.25), disables GPU for dF/F + xcorr, drops `nbins`, and coarsens the preprocess QC downsample (its one un-streamed buffer). The failed attempt's partial outputs are torn down race-free (mirror stopped, scratch + partial HDD rmtree'd) before the row is re-queued at the front, and the overrides are snapshotted/restored so they never leak into later normal rows. Disk-full is deliberately *not* retried (smaller batches can't free disk). Preprocess is itself largely OOM-safe (single-TIFF shift auto-falls-back to streaming; the grouped path is already streaming), so re-running from it is safe even though RAM OOM almost always originates in detection/suite2p.
...
**What to save (output footprint).** A **What to save** group (preset + three checkboxes, just under **Save figures during batch**) controls which large artifacts each recording keeps. The ROI arrays (`F`/`Fneu`/`spks`/`stat`/`iscell`/`ops`) and the dF/F + low-pass memmaps are **always kept** — every tab needs them to render. The **Keep everything** preset (default) preserves today's behavior; **Bare minimum (Calliope-only)** drops the registered `data.bin` and the shifted TIFF (artifacts used only for re-running detection / Tab 1 reload, not for rendering). The three checkboxes — **Keep suite2p registration (`data.bin`)** (default on), **Keep shifted / motion-corrected TIFF** (default on), **Archive raw TIFF (compressed)** (default off) — can be fine-tuned individually after picking a preset. Dropping registration means detection can't be re-run without re-registering; dropping the shifted TIFF means a Tab 1 reload must regenerate it. The dropped artifacts are still produced during the run (detection consumes them) then pruned at row finalize from both scratch and the HDD output, with the mirror skipping them so they're never copied across. Toggles are session-only (not persisted), matching **Save figures**. See `tabs/batch/README.md` for the full artifact map.
...
**Indicator / deconvolution τ.** In the Tab 3 header you pick the **GCaMP variant** from a dropdown, which sets Suite2p's OASIS deconvolution time constant τ (default **jGCaMP8m, τ = 0.25 s**; the other six cited variants run 0.15–1.5 s). A **custom τ (s)** field beside the dropdown overrides the variant for an indicator not in the preset list or a rig-calibrated value — enter a positive number, or leave it blank to use the dropdown. The same override is available per-recording in Tab 0's batch params (`gcamp_tau_custom`, 0 = use the dropdown). The τ actually used is written into `ops.npy` and surfaced in the export `manifest.json`, so a custom value stays reproducible.

**Interactive curation.** Click any ROI on the "All detected ROIs" panel to open a popout with five inspection panels (mean image, max projection, max + ROI, ROI footprint, ΔF/F trace) modeled on `roi_curation_app.py`. **Cell (1) / Not a cell (0)** flip `iscell.npy` in place and append to the curation CSV (`cellfilter.config.LABELS_CSV`, default the in-project `calliope/data/cellfilter_labels.csv`). A **Retrain cell filter** button stays disabled until the user has flipped at least one classification this session; clicking it spawns `cellfilter.train.main()` in a worker thread. A **Promote to filter mask** button (also gated on at least one flip) overlays this session's flips onto `predicted_cell_mask.npy` so downstream tabs (5/6/7, cross-correlation) honour the curator's labels *without* paying for a full retrain — CNN predictions for un-touched ROIs are preserved. Because the filtered dF/F memmap (`r0p7_filtered_dff.memmap.float32`) is column-sliced and anchored to `r0p7_cell_mask_bool.npy`, promote also rewrites that memmap (re-slicing from the full-ROI `r0p7_dff.memmap.float32`) and re-publishes the `plane0` channel, so memmap-reading tabs such as lowpass `n_kept` refresh immediately even on a reloaded completed run; already-computed results (events/clusters/xcorr) still need a re-run. Tab 3's panels 2 + 3 repaint immediately after every flip so the keep-set state stays in sync.
...
**Provenance sidecar.** Each recording writes a `<plane0>/meta.json` file that records exactly how it was produced — the registration, detection, extraction (including the GCaMP variant and τ), dF/F, low-pass, and spike-inference settings — as a single source of truth that downstream tools read. Writes are atomic and reads fall back to defaults, so older recordings without a sidecar still load.

**Z-drift QC.** After detection, CalLIOPE runs a quick check for axial (z) drift: it takes the first principal component across all ROIs' dF/F (the dominant shared mode) and flags abrupt step changes in it as candidate drift. It writes `zdrift_qc.png` (the trace with flagged frames marked) and `zdrift_pc1.csv` (so the figure is recreatable from a plain CSV), and records the verdict in `meta.json`. This only *flags* frames for you to judge — it never drops them, since dropping would require re-registering. This is the quantitative companion to the eyeball z-drift check on Tab 2.
...
**Post-detection archive.** Immediately after the prune, `core.detection_run.archive_recording_post_detection` collapses the recording to its smallest stable footprint: it locates the raw TIFF source(s) via the `_calliope_raw_paths.json` sidecar Tab 1 wrote, re-encodes each as a Zstd-19 lossless TIFF at `<rec>/<raw.name>.tif` (top level), verifies byte-equality after decompression before atomic-renaming into place, then deletes the now-redundant `shifted_*.tif` and the orphaned hardlink twin `detection/final/.../data.bin`. The encoder tries the horizontal-differencing predictor (`predictor=2`, ~2× better ratio on integer fluorescence) first and, if `tifffile` rejects the dtype (it raises for float raws) or the byte-equality check fails, transparently falls back to no predictor (`predictor=1`, still fully lossless under Zstd) before giving up — so a non-integer or otherwise awkward raw no longer aborts the whole archive. Per-recording disk drops from ~20 GB to ~6.5 GB (~50–60% size reduction on the raw). If you later need to re-detect, `load_existing_preprocess` regenerates the shifted from the compressed raw in ~1 min — total re-detect cost goes from ~5 min to ~6 min, the acceptable trade for the disk savings. Every step has an opt-out via `params` (`archive_post_detection`, `compress_raw_post_detection`, `delete_shifted_post_detection`, `delete_final_data_bin_post_detection`, `delete_external_raw_after_archive`, `raw_compression_level`); destructive deletion of the user's external raw at its source path is the only opt-in (off by default).
...
**Exporting figures.** Once detection finishes (or after "Load existing panels"), the **Export figures** button writes the detection ROI overlay panels (`all_rois.{png,svg}`, `kept_rois.{png,svg}`) to `<save_folder>/calliope_figures/detection/` along with a `manifest.json` at `<save_folder>/calliope_figures/manifest.json` capturing the calliope git SHA, the GCaMP variant, tau, fs, `pix_to_um`, cell counts, and the cellfilter checkpoint SHA-256. When a pixel scale is set (see the Pixel-scale params), the panels — live and exported — carry a white micrometre **scale bar** (the exported figures stay axis-off, microscopy-style); with no calibration they fall back to bare pixels. The export is blocked when the curation popout has un-promoted flips — promote first so the exported figures match what you're looking at. For the full per-stage figure set (detection + lowpass + events + clustering + crosscorrelation + spatial), run the recording through **Tab 0 — Batch runner**; `core/batch_pipeline.py` writes the same `calliope_figures/<stage>/` tree plus manifest as a normal part of the batch pipeline.

**Exporting to NWB.** An **Export NWB** button (enabled once a run finishes or after "Load existing panels") writes a DANDI-style `.nwb` for the recording: the imaging plane, per-ROI masks with their cell-probability and source, the fluorescence / neuropil / dF/F traces, the deconvolved spike estimate, and — when present — the population event windows. Imaging metadata comes from the recording's `meta.json`. NWB is an optional install (`pip install 'calliope[nwb]'`); without it the button reports that and does nothing. Tab 0's batch params expose the same export as an **Export NWB file** toggle so a whole queue can be written to NWB unattended.
...
1. **Per-ROI onsets.** For each cell, it computes a robust z-score of the derivative (using median + MAD instead of mean + std, which is robust to outliers — see Stern et al. 2024) and runs **hysteresis thresholding** (Schmitt trigger): an onset is recorded when the cell's derivative-z crosses up through the *enter* threshold (default `3.5`, robust-z/MAD units); the cell then stays active — suppressing further onsets — until its derivative-z drops below the *exit* threshold (default `1.5`), at which point the detector re-arms. Only counting a new onset after the cell has fully de-activated stops a trace hovering near threshold from registering repeated spurious onsets. Onsets within `min_sep_s` (default `0.1 s`) of the previous accepted onset in the same ROI are merged.
...
The popout also overlays the **circular-shift null floor** — the p95/p99 of candidate-peak prominences when each cell's onset train is independently time-shifted so any cross-cell coincidence is pure chance (`utils.circular_shift_null_prominences`, the same engine as `scripts/null_prominence_audit.py`). A real population event must clear that floor, so it's a principled lower bound for `min_prominence`. **By default this null now sets the floor automatically** (`auto_min_prominence`, p99) — detection estimates the prominence floor per recording from that recording's own null rather than using one fixed number. It's a deliberately weak, slightly-permissive estimate (it lands just below where a careful human would cut, so borderline events survive to curation), fitted against nine hand-tuned recordings where null p99 matched the manual median to within a few percent. It stays fully overridable: applying a value in this popout — or unticking `auto_min_prominence` in the Advanced dialog — switches back to the fixed `min_prominence`. (Supersedes the 2026-05-29 "reference only" decision, which rested on an n=3 audit later overturned at n=9.)
